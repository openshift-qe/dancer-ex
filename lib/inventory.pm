...
sub get_connection{
  ...
  my $dbh=DBI->connect("DBI:Pg:database=$ENV{'POSTGRESQL_DATABASE'};host=$db_host;port=$db_port",$ENV{'POSTGRESQL_USER'},$ENV{'POSTGRESQL_PASSWORD'}, { RaiseError => 1 } ) or die ("Couldn't connect to database: " . DBI->errstr );
  return $dbh;
}
